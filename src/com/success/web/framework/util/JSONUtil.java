...
public class JSONUtil {
	 ...
	 * @throws Exception
	 */
	public static JSONObject getJSONObject(Object dataCol,
			Collection<Object> excludeFields) throws JSONException {
		if (dataCol == null) {
	...
	public static JSONArray toJSONObject(Collection collection) {
		return toJSONObject(collection, null);
	}
	
	public static JSONArray toJSONObject(Collection pojoCollection, Collection excludeFields) {
		JSONArray jitems = new JSONArray();
		if (pojoCollection == null || pojoCollection.size() <= 0) {
			return jitems;
		}
		try {
			for (Iterator it = pojoCollection.iterator(); it.hasNext();) {
				Object obj = it.next();
				if (obj instanceof Map) {
					jitems.put(toJSON((Map) obj));
				} else {
