...
var isCheckAllStore = new Ext.data.ArrayStore({
	fields:['id','value'], data: [["1","仅查询当前组织"],["2","查询当前及下属组织"]]
});
var userIdCheckMenu = null;
var userIdCheckFun = null;
...
var funAccessStore = new Ext.data.JsonStore({
	remoteSort : true,
	root : 'rows',
	totalProperty : 'totalCounts',
	fields : [ "userFunId", "funId", "funName" ],
	proxy : new Ext.data.HttpProxy({
		url : contextPath+"/fun/queryUserFun.do?checkHaveAccess=''"
	})

});
funAccessStore.load();
var funAccessStore2 = new Ext.data.JsonStore({
	remoteSort : true,
	 root : 'rows',
	totalProperty : 'totalCounts',
	fields : [ "funId", "funName" ],
	proxy : new Ext.data.HttpProxy({
		url : contextPath+"/fun/queryFunByUserId.do?checkHaveAccess=not"
	})

});
funAccessStore2.load();
var funAccessStore3 = new Ext.data.JsonStore({
	remoteSort : true,
	root : 'rows',
	totalProperty : 'totalCounts',
	fields : [ "userFunId", "funId", "funName" ],
...
var userManger = (function() {
	...
	return {
		init : function() {
			...
			currentObj.funAccessPanel();
			currentObj.funAccessPanel2();
			currentObj.funAccessPanel3();
		},
...
//				title : '查询条件',
		...
		initAccessListGrid :function(){
			...
			var funAccessPanel1 = currentObj.funAccessPanel();
...
//				mode:'local',
			...
			funAccessPanel:function(){
			var funAccessGrid = new Ext.grid.GridPanel({
				id:'funAccessGridId',
				columns:[
				 {
				    header:'功能权限id',
				...
				store:funAccessStore,				
				height:200
			});
			var listGridStore = Ext.getCmp('funAccessGridId').store;
			listGridStore.on('beforeload', function(store) {
				if (listGridStore.lastOptions != null) {
					var param = {};
					param.userId = userIdCheckFun;
					listGridStore.baseParams = param;
				}
			});
			Ext.getCmp("funAccessGridId").getStore().reload();
			return funAccessGrid;
		},
		funAccessPanel2:function(){
			var sm2 = new Ext.grid.CheckboxSelectionModel(
//					{singleSelect : false}
				);
			var funAccessGrid2 = new Ext.grid.GridPanel({
				id:'funAccessGridId2',
//				title:'权限数据',
//				mode:'local',
				height:350,
				...
				store:funAccessStore2,				
				height:330
			});
			Ext.getCmp("funAccessGridId2").getStore().load();
			return funAccessGrid2;
		},
		funAccessPanel3:function(){
			var sm = new Ext.grid.CheckboxSelectionModel(
//					{singleSelect : true}
//					{dataIndex:'userFunId'}
				);
			var funAccessGrid3 = new Ext.grid.GridPanel({
				id:'funAccessGridId3',
//				title:'权限数据',
//				mode:'local',
				sm:sm,
				...
				store:funAccessStore3,				
				height:330
			});
			Ext.getCmp("funAccessGridId3").getStore().load();
			return funAccessGrid3;
		}
	};
...
function doQryFunAccess(){
	var rows=$("#userList").datagrid('getSelections');
	var userId=null;
	if(rows.length>0){
		userId = rows[0].userId;
	}
	userIdCheckFun = userId;
	
//	Ext.getCmp("funAccessGridId").store.removeAll();
//	Ext.getCmp("funAccessGridId").getStore().load({params:{
//		userId:userId
//	}});
	Ext.getCmp("funAccessGridId").getStore().reload();
}
function doCloseWin(){
	...
	var closeWFun = Ext.getCmp('funFormWindow');
	if(closeWFun!=null){
		closeWFun.close();
	}
};
...
function addFunAccess(){
	...
	funAccessStore2.load({params:{
		userId:userId
	}});
	var funAccessPanel2P = currentObj.funAccessPanel2();
	var formWin = new Ext.Window({
		id :'funFormWindow',
		title : '增加功能权限',
		width : 300,
		...
		buttons : [
					{
						...
						icon : '/IOMPROJ/public/image/确认.png',
						onClick : function() {
							var selinfo = Ext.getCmp('funAccessGridId2').getSelectionModel()
							.getSelections();
							if(selinfo.length<=0){
								Ext.Msg.alert("提示","请至少选择一条数据");
								return;
							...
							for(var i=0;i<selinfo.length;i++){
								if(i==0){
									funIdArray = selinfo[i].data.funId;
								}else{
									funIdArray += ","+selinfo[i].data.funId;
								}
							}
							Ext.Ajax.request({
								url : contextPath+"/fun/addUserFun.do",
								async : false,
								params:{  
									funIdArray:funIdArray,
									userId:userId
							    },
							    success : function(response, action) {
									Ext.Msg.alert('成功', Ext.decode(response.responseText).message);
						...
						icon : '/IOMPROJ/public/image/确认.png',
						onClick : function() {
							Ext.getCmp('funFormWindow').close();
						}
					} ]
...
function delFunAccess(){
	...
	funAccessStore3.load({params:{
		userId:userId
	}});
	var funAccessPanel3P = currentObj.funAccessPanel3();
	var formWin = new Ext.Window({
		id :'funFormWindow',
		title : '册除功能权限',
		width : 300,
		...
		buttons : [
					{
						...
						icon : '/IOMPROJ/public/image/确认.png',
						onClick : function() {
							var selinfo = Ext.getCmp('funAccessGridId3').getSelectionModel()
							.getSelections();
							if(selinfo.length<=0){
								Ext.Msg.alert("提示","请至少选择一条数据");
								return;
							...
							Ext.Ajax.request({
								url : contextPath+"/fun/delUserFun.do",
								async : false,
								params:{  
									funIdArray:funIdArray,
									userId:userId
