...
var agreementContent = (function() {
	...
	return {		
		initBasicPanel : function(){
			...
			for(var i = (0 + 8); i < (agreementTacheArray.length + 8); i ++){
				items[i] =  generBasicObj(agreementTacheArray[i - 8].tacheName, agreementTacheArray[i - 8].userName);
			}
...
function doSubmit(){
	...
	Ext.Ajax.request( {
		url : contextPath+'/agreement/eidtAgreement.do',
		waitTitle : '提示', 
 		waitMsg: '请稍后,正在提交数据...',
		params : {
			orderId : orderId,
