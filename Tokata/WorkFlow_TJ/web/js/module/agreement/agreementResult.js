...
Ext.onReady(function() {
	var basicPanel = agreementResult.initBasicPanel(); 
	var mainPanel = agreementResult.initMainPanel();
	var buttonPanel = agreementResult.initButtonPanel();
...
//页面配置对象
var agreementResult = (function() {
	 
	return {		
		initBasicPanel : function(){
			var items = new Array();
			var agreement = Ext.decode(agreementJSON);
			items = [{ 
		            columnWidth:.25,
		            layout:'form',
		            items:[{ 
...
function generMainObj(orderId, agreementContent){
	...
	obj[4] = agreementContent.problem;
	obj[5] = agreementContent.improve;
...
function doSubmit(){
	Ext.Ajax.request( {
		url : contextPath+'/agreement/agreementResultOK.do',
		waitTitle : '提示', 
 		waitMsg: '请稍后,正在提交数据...',
		params : {
			orderId : orderId,
...
function doCancle(){
	var resultMessage = "";
	Ext.MessageBox.prompt("输入框","请输入结论：",function(bu,txt){
		if("cancel"==bu) return;
		resultMessage= txt;  
		Ext.Ajax.request( {
			url : contextPath+'/agreement/agreementResultNG.do',
			waitTitle : '提示', 
	 		waitMsg: '请稍后,正在提交数据...',
			params : {
				orderId : orderId,
